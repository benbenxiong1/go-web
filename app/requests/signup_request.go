...
// ValidateSignupEmailExist 验证用户邮箱是否存在
...
// SignupUsingEmailRequest 通过用户邮箱注册的请求信息
...
func SignupUsingEmail(data interface{}) map[string][]string {
	rules := govalidator.MapData{
		"name":             []string{"required", "between:3:20", "alpha_num", "not_exists:user,name"},
